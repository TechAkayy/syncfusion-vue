<script>
  ...
  export default {
    ...
    data: () => {
      return {
        ddData: [
          {value: 1000, text: '1,000 Rows and 11 Columns'},
          {value: 10000, text: '10,000 Rows and 11 Columns'},
        ],
        ddValue: 1000,
        stTime: null,
        loadTime: null,
        ...
        coltemplate: function () {
          return {
            template: createApp({}).component('coltemplate', {
              template:
                '<div><img src="https://ej2.syncfusion.com/vue/demos/src/grid/images/Map.png" class="e-image" :alt="data.Location"/> &nbsp<span id="locationtext">{{data.Location}}</span></div>',
              data: function () {
                return {data: {}}
              },
            }),
        ...
        statusTemplate: function () {
          return {
           ...
           <span :class="{'statustxt': true, 'e-activecolor': data.Status == active, 'e-inactivecolor': data.Status == inactive}">{{data.Status}}</span></div>`,
              data: function () {
                return {data: {}, active: 'Active', inactive: 'Inactive'}
              },
            }),
          }
        },
        trustTemplate: function () {
          return {
            template: createApp({}).component('trustTemplate', {
              template:
                '<div><img style="width: 31px; height: 24px" :src="image" :alt="data.Trustworthiness" /><span id="Trusttext">{{data.Trustworthiness}}</span></div>',
              data: function () {
                return {
                  data: {},
                  Perfect: 'Perfect',
    ...
    methods: {
      valueChange: function (args) {
        ...
        this.stTime = null
        let contentElement =
          this.$refs.overviewgrid.ej2Instances.contentModule.content
      ...
      load: function () {
        let proxy = this
        this.$refs.overviewgrid.$el.ej2_instances[0].on(
          'data-ready',
          function () {
            proxy.dReady = true
          }
        )
        this.$refs.overviewgrid.$el.addEventListener(
          'DOMSubtreeModified',
          function () {
    ...
    <div class="content-wrapper">
      <div style="padding-bottom: 18px">
        <ejs-dropdownlist
          ...
          :fields="fields"
          :change="valueChange"
          :value="ddValue"></ejs-dropdownlist>
      ...
      <ejs-grid
        ...
        <e-columns>
          ...
          <e-column
            ...
            width="200"
            :template="trustTemplate"></e-column>
          <e-column
            field="Rating"
            headerText="Rating"
            width="160"
          ...
          <e-column
            ...
            width="180"
            :template="progessTemplate"></e-column>
          <e-column
            field="CurrentSalary"
...
<style scoped>
  @import '@/assets/styles/Grid/style.css';
</style>
